...
RUN yum -y install which && /usr/local/lib/nimbix_desktop/install-centos-tiger.sh && yum clean all && ln -s /usr/local/lib/nimbix_desktop /usr/lib/JARVICE/tools/nimbix_desktop && systemctl enable nimbix_desktop && dbus-uuidgen >/etc/machine-id && rm -f /etc/xdg/autostart/xfce-polkit.desktop
